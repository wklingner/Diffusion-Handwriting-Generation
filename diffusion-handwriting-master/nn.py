...
class StyleExtractor(Model):
    def __init__(self, weights_path=None):
        super().__init__()
        self.mobilenet = MobileNetV2(include_top=False, pooling=None, weights=None, input_shape=(96, 96, 3))
        if weights_path is not None:
            self.mobilenet.load_weights(weights_path)
...
class DiffusionWriter(Model):
    ...
    def load_partial_weights(self, weights_path):
        # Load weights for specific layers
        self.text_style_encoder.load_weights(weights_path, by_name=True)
        self.att_dense.load_weights(weights_path, by_name=True)
        for att_layer in self.att_layers:
            att_layer.load_weights(weights_path, by_name=True)

        super(DiffusionWriter, self).__init__()

    def call(self, strokes, text, alpha, sigma, style_vector, training=False):
        ...
        attention_map = att  # Assuming att is the attention map you want to return
        return output, pen_lifts, attention_map
